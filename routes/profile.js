...
router.put("/profile/:id/update",middleware.checkOwnership, function (req, res) {
    ...
    if (age < 15 || age > 76) {
        req.flash("error", "Age " + age + " is restricted. You must be 16 years or older.");
...
router.put("/profile/:id/view",upload.array("images"),async function(req,res){
...
router.put("/profile/:id/view1", upload.array("images"), async function (req, res) {
    User.findById(req.params.id, async function (err, user) {
        //check if there is any image for deletion
        if (req.body.deleteImages && req.body.deleteImages.length) {
            //assign deleteImages from req.body to its own variable
