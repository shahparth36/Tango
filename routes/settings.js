...
router.post("/edit/:id",middleware.isLoggedIn, (req, res) => {
    ...
    , async (err, updatedUser) => {
        if (err) {
            if (err.message === `E11000 duplicate key error collection: dating_app.users index: username_1 dup key: { username: "${req.body.username}" }`) {
                req.flash("error", "Username already exists.Please try again.");
                return res.redirect(`/settings/${req.params.id}`);
            }

            if (err.message === `E11000 duplicate key error collection: dating_app.users index: email_1 dup key: { email: "${req.body.email}" }`) {
                req.flash("error", "Email already exists.Please try again.");
                return res.redirect(`/settings/${req.params.id}`);
            }
            
            if (err.message === `E11000 duplicate key error collection: dating_app.users index: phNumber_1 dup key: { phNumber: "${req.body.phNumber}" }`) {
            ...
            if (req.body.password === req.body.confirm) {
                req.user.setPassword(req.body.password, function (err) {
                    req.user.save(function (err) {
                        req.flash("success", "Your password has been changed successfully.Please login to continue.")
                        res.redirect("/login");
                  });
                });
                } else {
